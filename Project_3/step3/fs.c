...
typedef struct 
{
    int rw;//0 for r, 1 for w ;2 for formatted; 3 for move; 4 for clear
    int beginPos;
    int size;
    int oriPos;
    //char data[2048];
...
void init_SONDIRECTORYTABLE(int blockPos)
{
    ...
    void *buf = &stb;
    writeDisk((BLOCKBASE + blockPos) * BLOCKSIZE, buf, BLOCKSIZE);
}
...
void init_SINGLEINDIRECT(int blockPos)//for the available, 0 for not exist, 1 for can be used , -1 for full
{
    ...
    char *buf = (char*) &sid;
    writeDisk((BLOCKBASE + blockPos)*BLOCKSIZE, buf, 256);
}   
...
void setBitMap(int type, int N)
{
    ...
    char* buf = (char*)&bmp;
    writeDisk(1 * BLOCKSIZE, buf, sizeof(bmp));
}
...
void returnBitMap(int type, int N)
{
    ...
    if (type == 0)
    {
        ...
        sb.s_free_inodes_count++;
        clearDisk(INODEBASE * BLOCKSIZE + N * INODESIZE, 0, INODESIZE);
    }
    if (type == 1)
    {
        bmp.blockMap[N >> SHIFT ] |= 0 << (N & 0x07);
        sb.s_free_blocks_count++;
        clearDisk((BLOCKBASE + N) * BLOCKSIZE, 0, BLOCKSIZE);       
    }
    char* buf = (char*)&bmp;
    writeDisk(1 * BLOCKSIZE, buf, sizeof(bmp));
}
...
int addFiletoDirectory(int fileInode, char* fileName)
{
    ...
    if (currentDirectory.i_uid <= 7)//can store in the first block;
    {
        ...
        writeDisk( (BLOCKBASE + currentDirectory.i_direct[0])*BLOCKSIZE + currentDirectory.i_uid * 32, buf, sizeof(cd));
        currentDirectory.i_size += sizeof(cd);
        readDisk((char*)&cd, (BLOCKBASE + currentDirectory.i_direct[0])*BLOCKSIZE + currentDirectory.i_uid * 32, sizeof(cd));
        //printf("size: %ld\n", sizeof(cd));
        //printf("cd: %u, %s\n",cd.f_inodeNum, cd.f_fileName);
    ...
    else if (currentDirectory.i_uid > 7 && currentDirectory.i_uid <= 31)//store in other 3 direct block
    {
        ...
        writeDisk((BLOCKBASE + currentDirectory.i_direct[directnum])*BLOCKSIZE + offset * 32, buf, sizeof(cd));
        //currentDirectory.i_size += sizeof(cd);
        //printf("here %u, %u\n", directnum, offset);
    ...
    else //store in sindirect
    {
            ...
            readDisk((char*)&sid, (BLOCKBASE + currentDirectory.i_sindirect) * BLOCKSIZE, sizeof(sid));
            ...
            if(sid.s_blocknum[idm] == -1)// not exist
            {
                ...
                char *buf = (char*) &sid;
                writeDisk((BLOCKBASE + currentDirectory.i_sindirect) * BLOCKSIZE, buf, 256);
            }
            
            writeDisk((BLOCKBASE + sid.s_blocknum[idm]) * BLOCKSIZE + offset * 32 , buf, sizeof(cd));
            currentDirectory.i_size += sizeof(cd);
    }
    currentDirectory.i_uid ++;
    char *bufDir = (char*)&currentDirectory;
    writeDisk(INODEBASE*BLOCKSIZE + currentDirectoryInode*INODESIZE, bufDir, INODESIZE);
    return 0;   
}
...
void init_FS(char* diskfile)
{
    ...
    buf = (char*) &root;
    //save Inode!!!
    writeDisk(INODEBASE * BLOCKSIZE, buf, INODESIZE);
    currentDirectoryInode = 0;
    readDisk((char*)&currentDirectory,INODEBASE * BLOCKSIZE, sizeof(currentDirectory));
    char name[]= "root";
...
int createFile(char* fileName) //-1 for error
{
    ...
    writeDisk((INODEBASE * BLOCKSIZE + inodePos * INODESIZE), buf, INODESIZE);
...
int fatherLink (int sonInode)// 1 for error
{
    ...
    readDisk((char*)&stb, (BLOCKBASE + currentDirectory.i_dindirect) * BLOCKSIZE, sizeof(stb));
    ...
    char *buf = (char*) &stb;
    writeDisk((BLOCKBASE + currentDirectory.i_dindirect) * BLOCKSIZE, buf, BLOCKSIZE);
    return 0;
}
...
int createDirectory(char* directoryName) //1 for error
{
    ...
    writeDisk(INODEBASE * BLOCKSIZE + currentDirectoryInode * INODESIZE, buf, INODESIZE);
    
    currentDirectoryInode = newdirect.i_id;
    readDisk((char*)&currentDirectory, INODEBASE * BLOCKSIZE + currentDirectoryInode * INODESIZE, INODESIZE);//update
    //printf("uid:%u.\n", currentDirectory.i_uid);
    ...
    readDisk((char*)&currentDirectory, INODEBASE * BLOCKSIZE + currentDirectoryInode * INODESIZE, INODESIZE);
...
int deleteFile(char * deleteFile)
{
    ...
    for (loc = 0; loc < currentDirectory.i_uid; loc++) //first block hold 8, from 31 to i_uid hold the rest
    {
        ...
        if (loc < 32)
        {
            readDisk((char*)&fd, (BLOCKBASE + currentDirectory.i_direct[loc/8]) * BLOCKSIZE + 32 * (loc % 8), sizeof(fd));
        ...
        else
        {
            ...
            readDisk((char*)&sid, (BLOCKBASE + currentDirectory.i_sindirect) * BLOCKSIZE, sizeof(sid));
            int offset = (loc - 31) / 8;
            readDisk((char*)&fd, (BLOCKBASE + sid.s_blocknum[offset]) * BLOCKSIZE + (loc % 8) * 32, sizeof(fd));
    ...
    INODE tarI;
    readDisk((char*)&tarI, INODEBASE * BLOCKSIZE + fd.f_inodeNum * INODESIZE, INODESIZE);
    for (int i = 0; i < 4; i++)
    {
    ...
    {
        SINGLEINDIRECT sid;
        readDisk((char*)&sid, (BLOCKBASE + tarI.i_sindirect) * BLOCKSIZE, sizeof(sid));
        for (int j = 0; j < 8; j++)
        {
        ...
        char* buf = (char*) &sid;
        writeDisk((BLOCKBASE + tarI.i_sindirect) * BLOCKSIZE, buf, BLOCKSIZE);
    ...
    for (reLoc; reLoc < currentDirectory.i_uid; reLoc ++)
    {
        ...
        {
            FILEDIRECTORY tmp;
            readDisk((char*)&tmp,(BLOCKBASE + currentDirectory.i_direct[tarLoc/8]) * BLOCKSIZE + (tarLoc % 8 ) * 32, sizeof(FILEDIRECTORY));
            moveDisk((BLOCKBASE + currentDirectory.i_direct[tarLoc/8]) * BLOCKSIZE + (tarLoc % 8 ) * 32,
             (BLOCKBASE + currentDirectory.i_direct[reLoc / 8]) * BLOCKSIZE + (reLoc % 8) * 32 , sizeof(FILEDIRECTORY));
            clearDisk((BLOCKBASE + currentDirectory.i_direct[reLoc / 8]) * BLOCKSIZE + (reLoc % 8) * 32, 0, sizeof(FILEDIRECTORY));
            readDisk((char*)&tmp, (BLOCKBASE + currentDirectory.i_direct[tarLoc/8]) * BLOCKSIZE + (tarLoc % 8 ) * 32, sizeof(FILEDIRECTORY));
        ...
        else if (reLoc == 32 && tarLoc == 31)// reloc strored in sindirct(block) --> sid --> blocknum[0] --> block
        {
            ...
            readDisk((char*)&sid, (BLOCKBASE + currentDirectory.i_sindirect) * BLOCKSIZE, sizeof(sid));
            moveDisk((BLOCKBASE + currentDirectory.i_direct[tarLoc/8]) * BLOCKSIZE + (tarLoc % 8 ) * 32,
            (BLOCKBASE + sid.s_blocknum[0]) * BLOCKBASE + 0 * 32, sizeof(FILEDIRECTORY));
            clearDisk((BLOCKBASE + sid.s_blocknum[0]) * BLOCKSIZE + 0 * 32, 0, sizeof(FILEDIRECTORY));
        }
        else
        {
            SINGLEINDIRECT sid;
            readDisk((char*)&sid,(BLOCKBASE + currentDirectory.i_sindirect) * BLOCKSIZE, sizeof(sid));
            moveDisk((BLOCKBASE + sid.s_blocknum[(tarLoc - 32) / 8]) * BLOCKSIZE + ((tarLoc - 32) % 8) * 32, 
            (BLOCKBASE + sid.s_blocknum[(reLoc -32) / 8]) * BLOCKSIZE + ((reLoc - 32) % 8) * 32, sizeof(FILEDIRECTORY));
            clearDisk((BLOCKBASE + sid.s_blocknum[(reLoc -32) / 8]) * BLOCKSIZE + ((reLoc - 32) % 8) * 32, 0, sizeof(FILEDIRECTORY));
    ...
    || currentDirectory.i_uid == 73 || currentDirectory.i_uid == 81 || currentDirectory.i_uid == 89)
    {
        ...
        char* buf = (char*) &sid;
        writeDisk(( BLOCKBASE + currentDirectory.i_sindirect) * BLOCKSIZE, buf, sizeof(sid));
    }
    //dont forget uid --
    currentDirectory.i_uid --;
    return 0;
    ...
    else
    {
        ...
        {   
            ...
            if (sdInode == -1)
                continue;
            INODE sd;
            readDisk((char*)&sd, INODEBASE * BLOCKSIZE + sdInode * INODESIZE, sizeof(sd));
            //printf("sd.i_id: %u\n", sd.i_id);
            FILEDIRECTORY sdName;
            readDisk((char*)&sdName, (BLOCKBASE + sd.i_direct[0]) * BLOCKSIZE, sizeof(sdName));
...
int deleteDir(char* name)
{
    ...
    {
        readDisk((char*)&fd, (BLOCKBASE + currentDirectory.i_direct[0]) * BLOCKSIZE + 32, sizeof(fd));
    ...
    if (currentDirectory.i_dindirect != -1)
    {
        ...
        for (int i = 0; i < 32 || delNum < stb.num; i++)
        {
            if (stb.sonDirectory[i] != -1)//delete sondir
            {
                //get the name 
                INODE sonDir;
                readDisk((char*)&sonDir, INODEBASE * BLOCKSIZE + stb.sonDirectory[i] * INODESIZE, INODESIZE);
                FILEDIRECTORY sfd;
                readDisk((char*)&sfd, (BLOCKBASE + sonDir.i_direct[0]) * BLOCKSIZE, sizeof(sfd));
        ...
        buf = (char*) &stb;
        writeDisk((BLOCKBASE + currentDirectory.i_dindirect) * BLOCKSIZE, buf, sizeof(stb));
...
int listing(FILE* fp, int newsockfd1)
{
    FILEDIRECTORY fd;
    INODE file;
    readDisk((char*)&fd, (BLOCKBASE + currentDirectory.i_direct[0]) * BLOCKSIZE, sizeof(fd));
    ...
    for (int i = 1; i < currentDirectory.i_uid; i++)
    {
        if (i < 32)
        {
            readDisk((char*)&fd, (BLOCKBASE + currentDirectory.i_direct[i/8]) * BLOCKSIZE + (i%8) * 32, sizeof(fd));
            
            readDisk((char*)&file, (INODEBASE * BLOCKSIZE + fd.f_inodeNum * INODESIZE), INODESIZE);
            
            printf("%s filesize: %u\n", fd.f_fileName, file.i_size);
            
            tmpSize = strlen(fd.f_fileName);
            pass = (char*)realloc(pass, size + tmpSize + 1);
            strcat(pass, fd.f_fileName);
            size += tmpSize;

            tmpSize = strlen(" filesize ");
            pass = (char*)realloc(pass, size + tmpSize + 1);
            strcat(pass, " filesize ");
            size += tmpSize;
            char numberStr[20]; // Assuming the number won't exceed 20 characters
            sprintf(numberStr, "%u", file.i_size);
            pass = realloc(pass, size + strlen(numberStr) + 1);
            strcat(pass, numberStr);
            size += strlen(numberStr);
            tmpSize = strlen(" ");
            pass = (char*)realloc(pass, size + tmpSize + 1);
            strcat(pass, " ");
            size += tmpSize;
            ...
            tmpSize = strlen(time_str);
            pass = (char*)realloc(pass, size + tmpSize + 1);
            strcat(pass, time_str);
            size += tmpSize;
            tmpSize = strlen(" ");
            pass = (char*)realloc(pass, size + tmpSize + 1);
            strcat(pass, " ");
        ...
        else 
        {
            SINGLEINDIRECT sid;
            readDisk((char*)&sid, (BLOCKBASE + currentDirectory.i_sindirect) * BLOCKSIZE, sizeof(sid));
            readDisk((char*)&fd, (BLOCKBASE + sid.s_blocknum[(i - 32)/8]) * BLOCKSIZE + ((i-32)%8) * 32, sizeof(fd));
            printf("%s\n", fd.f_fileName);
        }
    }
    printf("SonDirectory:\n");
    if (currentDirectory.i_dindirect != -1)
    {
        ...
        while (tmp < stb.num)
        {
            ...
            {
                INODE sd;
                readDisk((char*)&sd, (INODEBASE * BLOCKSIZE + stb.sonDirectory[i] * INODESIZE), INODESIZE);
                
                FILEDIRECTORY cd;
                readDisk((char*)&fd, (BLOCKBASE + sd.i_direct[0]) * BLOCKSIZE, sizeof(cd));
    ...
    strcat(pass, "\n");
    size += tmpSize;
    int n = write (newsockfd1, pass, strlen(pass));
    if (n < 0)
...
int catFile(char* name, FILE* fp, int newsock)
{
    ...
    else
    {
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + tar.i_direct[0]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
            strcat(pass, dataBuf);
            size += tmpSize;
            tmpSize = strlen("\n");
            ...
            strcat(pass, "\n");
            size += tmpSize;
            int n = write (newsock, pass, strlen(pass));
            if (n < 0)
                herror("Error: fail to pass\n");
            return 0;
        }
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf,(BLOCKBASE + tar.i_direct[0]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + tar.i_direct[1]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + tar.i_direct[1]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + tar.i_direct[2]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + tar.i_direct[2]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + tar.i_direct[3]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + tar.i_direct[3]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[0]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[0]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[1]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[1]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[2]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[2]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[3]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[3]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[4]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[4]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[5]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[5]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[6]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[6]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        if (restSize <= 256)
        {
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[7]) * BLOCKSIZE, restSize);
            printf("%s\n", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
        ...
        {
            restSize -= 256;
            readDisk((char*)&dataBuf, (BLOCKBASE + sid.s_blocknum[7]) * BLOCKSIZE, 256);
            printf("%s", dataBuf);
            fprintf(fp, "%s",dataBuf);
            tmpSize = strlen(dataBuf);
            pass = (char*)realloc(pass, size + tmpSize + 1);
    ...
    char* buf = (char*) &tar;
    writeDisk(INODEBASE * BLOCKSIZE + result * INODESIZE, buf, INODESIZE);
    return 0;
}
...
{
    INODE tar;
    readDisk((char*)&tar, INODEBASE * BLOCKSIZE + inodeNum * INODESIZE, INODESIZE);
    for (int i = 0; i < 4; i++)
    {
        if (tar.i_direct[i] != -1)
        {
    ...
    if (tar.i_sindirect != -1)
    {
        ...
        char* buf = (char*) &sid;
        writeDisk((BLOCKBASE + tar.i_sindirect) * BLOCKSIZE, buf, BLOCKSIZE);
    ...
    tar.i_size = 0;
    char*buf = (char*)&tar;
    writeDisk(INODEBASE * BLOCKSIZE + inodeNum * INODESIZE, buf, INODESIZE);
}
...
int writeFile(int inodeNum, int len, char* data, int beginPos)
{
    ...
    if (endLoc >=4)
    {
        ...
        char* buf = (char*)&sid;
        writeDisk((BLOCKBASE + tar.i_sindirect) * BLOCKSIZE, buf, sizeof(sid));
    }
    ...
    while(restLen > 0)
    {
        char*buf = (char*)&data[dataPos];
        writeDisk((BLOCKBASE + array[tmp]) * BLOCKSIZE + firstPos, buf, min(256 - currentPos % 256, restLen));
        firstPos = 0;
...
int deleteIn(char* name, int pos, int len)
{
    ...
    if (end >= tar.i_size - 1)//delete till the end of the file
    {
        ...
        if (beginLoc < 4)//delete sid
        {
            ...
            clearDisk((BLOCKBASE + tar.i_direct[beginLoc]) * BLOCKSIZE + beginPos, 0, 256 - beginPos);
        }
        else//in the sid
        {
            if (tar.i_sindirect == -1)
            ...
            int beginPos = pos%256;//relative in a block
            clearDisk((BLOCKBASE + sid.s_blocknum[beginLoc - 4]) * BLOCKSIZE + beginPos, 0, 256 - beginPos);
            char* buf = (char*) &sid;
            writeDisk((BLOCKBASE + tar.i_sindirect) * BLOCKSIZE, buf, sizeof(sid));
        }
    ...
    if (copyLoc == copyEndLoc)
    {
        if (copyLoc < 4)
        {
            readDisk((char*)&buf,(BLOCKBASE + tar.i_direct[copyLoc]) * BLOCKSIZE + copyPos, copyEndPos - copyPos + 1);
        ...
        else
        {
            SINGLEINDIRECT sid;
            readDisk((char*)&sid, (BLOCKBASE + tar.i_sindirect) * BLOCKSIZE, sizeof(sid));
            readDisk((char*)&buf,(BLOCKBASE + sid.s_blocknum[copyLoc - 4]) * BLOCKSIZE + copyPos, copyEndPos - copyPos + 1);
            locInBuf += copyEndPos - copyPos + 1;
        }
    }
    else
    {
        for (tmpLoc; tmpLoc < copyEndLoc; tmpLoc++)// the last Loc manipulated out of FOR
        {
            if (tmpLoc < 4)
            {
                readDisk((char*)&buf[locInBuf], (BLOCKBASE + tar.i_direct[tmpLoc]) * BLOCKSIZE + tmpPos%256, 256 - tmpPos%256);
            ...
            else
            {
                SINGLEINDIRECT sid;
                readDisk((char*)&sid, (BLOCKBASE + tar.i_sindirect) * BLOCKSIZE, sizeof(sid));
                readDisk((char*)&buf[locInBuf], (BLOCKBASE + sid.s_blocknum[tmpLoc - 4]) * BLOCKSIZE + tmpPos%256, 256 - tmpPos%256);
                locInBuf += 256 - tmpPos%256;
                tmpPos = 0;
            }
        }
        ...
        if (copyEndLoc < 4)
        {
            readDisk((char*)&buf[locInBuf], (BLOCKBASE + tar.i_direct[copyEndLoc]) * BLOCKSIZE, copyEndLoc%256);
        ...
        else
        {
            SINGLEINDIRECT sid;
            readDisk((char*)&sid, (BLOCKBASE + tar.i_sindirect) * BLOCKSIZE, sizeof(sid));
            readDisk((char*)&buf[locInBuf], (BLOCKBASE + sid.s_blocknum[copyEndLoc]) * BLOCKSIZE, copyEndLoc%256);
            locInBuf += copyEndLoc%256;
        }   
    }
    //use writeIn()
    ...
    if (garbageLoc < 4)//delete sid
    {
        ...
        int beginPos = pos%256;//relative in a block
        clearDisk((BLOCKBASE + tar.i_direct[garbageLoc]) * BLOCKSIZE + garbagePos, 0, 256 - garbagePos);
    }
    else//in the sid
    {
        if (tar.i_sindirect == -1)
        ...
        int beginPos = pos%256;//relative in a block
        clearDisk((BLOCKBASE + sid.s_blocknum[garbageLoc - 4]) * BLOCKSIZE + garbagePos, 0, 256 - garbagePos);
        char* buf = (char*) &sid;
        writeDisk((BLOCKBASE + tar.i_sindirect) * BLOCKSIZE, buf, sizeof(sid));
    }
    ...
    writeDisk(INODEBASE * BLOCKSIZE + result * INODESIZE, buffer, INODESIZE);
    return 0;
}
...
int main(int argc, char** argv)
{
    ...
    int s, c;
    //handle socket
    int sockfd1, newsockfd1, portno1, n;
    int  portno2, m;
    struct sockaddr_in serv_addr2;
    struct hostent *server2;
    socklen_t clilen1;
    //char command1[MAX_COMMANDS] = {0};
    ...
    serv_addr2.sin_family = AF_INET;
    bcopy((char *)server2->h_addr_list[0], (char *)&serv_addr2.sin_addr.s_addr, server2->h_length);
    ...
    while (1)
    {
        ...
        */
        ...
        else if(line[0] == 'c' && line[1] =='d' && line[2] == ' ')
        {
            ...
            INODE cd;
            readDisk((char*)&cd, INODEBASE * BLOCKSIZE + currentDirectoryInode * INODESIZE, INODESIZE);
            FILEDIRECTORY cdName;
            readDisk((char*)&cdName, (BLOCKBASE + cd.i_direct[0]) * BLOCKSIZE, sizeof(cdName));
        ...
        else if (line[0] == 'c' && line[1] == 'a' && line[2] == 't' && line[3] == ' ')
        {
            ...
            sscanf(line, "cat %[^\n]", file);
            res =  catFile(file, fp, newsockfd1);
            if (res != 0)
            {
        ...
        else if (line[0] == 'w' && line[1] == ' ')
        {
            ...
            else
            {
                ...
                INODE tar;//remember to save
                readDisk((char*)&tar, INODEBASE * BLOCKSIZE + node * INODESIZE, INODESIZE);
                readDisk((char*)&tmp, (BLOCKBASE + tar.i_direct[0])* BLOCKSIZE, len);
                printf("%s\n", tmp);
                fprintf(fp,"Yes\n");
                int n = write (newsockfd1, "Yes\n", strlen("Yes\n"));
                if (n < 0)
